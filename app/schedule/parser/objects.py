...
class GroupScheduleParser(ScheduleParser):
    ...
    def _collect_group_url(self) -> None:
        ...
        if response and response.status_code == 200:
            ...
            if group_link:
                self.parsing_storage['group_url'] = join_url(self.base_url, group_link.get('href'))

    @staticmethod
    def _parse_schedule(soup: bs4.BeautifulSoup, week_monday: date) -> Dict[date, List[List[str]]]:
        week_schedule = {}
...
class TeacherScheduleParser(ScheduleParser):
    ...
    def _parse_schedule(self, soup: bs4.BeautifulSoup, week_monday: date) -> Dict[date, List[List[str]]]:
        week_schedule = {}
        trs = soup.select('tr')
